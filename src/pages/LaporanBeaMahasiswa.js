...
// @mui
import { Stack, Button, Typography, Container, Icon, Alert, AlertTitle, TextField, Grid, FormControl } from '@mui/material';
...
export default function LaporanBeaMahasiswa() {
    // const theme = useTheme();
    ...
    const handleSubmit = async (e) => {
      ...
      console.log(nrp)
      await getBeaApply(nrp);
      console.log('masuk submit');
    };

    const getBeaApply = async(nrp) => {
      console.log("sukses masuk");
      const token = Cookies.get("myToken");
      console.log(`${process.env.REACT_APP_BACKEND_URL}/laporan/beamhs/${nrp}`);
      await axios.get(`${process.env.REACT_APP_BACKEND_URL}/laporan/beamhs/${nrp}`,
      {
        headers: {
      ...
      .catch(error => {
          navigate(`/laporan/beamahasiswa/mhs`);
    ...
    const handleOnExport = () => {
      ...
      XLSX.writeFile(wb, `laporan_mahasiswa_${beaapply[0].mhs_nrp}.xlsx`)
    };
    ...
    const handleOnPDF = () => {
      ...
      doc.autoTable({
        head: [['Jenis Beasiswa', 'Periode', 'Waktu Pendaftaran', 'Status Pendaftaran']],
        body: dataToExport.map(item => [
          item['Jenis Beasiswa'],
          item.Periode,
          item['Waktu Pendaftaran'],
          item['Status Pendaftaran'],
        ]),
      });
  
      doc.save(`laporan_mahasiswa_${beaapply[0].mhs_nrp}.pdf`);
    };
